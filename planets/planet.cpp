...
float getVecLen(glm::vec3 vec) {
    return std::pow((std::pow(vec[0], 2) +
                     std::pow(vec[1], 2) +
                     std::pow(vec[2], 2)), 0.5);
}
...
void Planet::createObject() {
    ...
    positions.push_back(glm::vec3(float(std::pow((8.0 / 9.0), 0.5)), -(1.0 / 3.0), 0));
    positions.push_back(glm::vec3(-(std::pow((2.0 / 9.0), 0.5)), -(1.0 / 3.0), std::pow((2.0 / 3.0), 0.5)));
    positions.push_back(glm::vec3(-(std::pow((2.0 / 9.0), 0.5)), -(1.0 / 3.0), -(std::pow((2.0 / 3.0), 0.5))));
    //face 1
    glm::vec3 pos;
    for (int i = 0; i < 2; ++i) {
        pos = glm::vec3((positions[i][0] + positions[(i + 1) % 3][0]) / 2.0,
                        (positions[i][1] + positions[(i + 1) % 3][1]) / 2.0,
                        (positions[i][2] + positions[(i + 1) % 3][2]) / 2.0);
        float len = getVecLen(pos);
        pos = glm::vec3(pos[0] / len, pos[1] / len, pos[2] / len);
    ...
    positions.push_back(glm::vec3((positions[0][0] + positions[2][0]) / 2.0,
                                  (positions[0][1] + positions[2][1]) / 2.0,
                                  (positions[0][2] + positions[2][2]) / 2.0));
    positions.push_back(glm::vec3((positions[1][0] + positions[2][0]) / 2.0,
                                  (positions[1][1] + positions[2][1]) / 2.0,
                                  (positions[1][2] + positions[2][2]) / 2.0));
    // 2
    positions.push_back(glm::vec3((positions[0][0] + positions[1][0]) / 2.0,
                                  (positions[0][1] + positions[1][1]) / 2.0,
                                  (positions[0][2] + positions[1][2]) / 2.0));
    positions.push_back(glm::vec3((positions[1][0] + positions[3][0]) / 2.0,
                                  (positions[1][1] + positions[3][1]) / 2.0,
                                  (positions[1][2] + positions[3][2]) / 2.0));
    positions.push_back(glm::vec3((positions[3][0] + positions[0][0]) / 2.0,
                                  (positions[3][1] + positions[0][1]) / 2.0,
                                  (positions[3][2] + positions[0][2]) / 2.0));
    //3
    positions.push_back(glm::vec3((positions[0][0] + positions[2][0]) / 2.0,
                                  (positions[0][1] + positions[2][1]) / 2.0,
                                  (positions[0][2] + positions[2][2]) / 2.0));
    positions.push_back(glm::vec3((positions[2][0] + positions[3][0]) / 2.0,
                                  (positions[2][1] + positions[3][1]) / 2.0,
                                  (positions[2][2] + positions[3][2]) / 2.0));
    positions.push_back(glm::vec3((positions[3][0] + positions[0][0]) / 2.0,
                                  (positions[3][1] + positions[0][1]) / 2.0,
                                  (positions[3][2] + positions[0][2]) / 2.0));
    //4
    ...
    positions.push_back(glm::vec3((positions[2][0] + positions[3][0]) / 2.0,
                                  (positions[2][1] + positions[3][1]) / 2.0,
                                  (positions[2][2] + positions[3][2]) / 2.0));
    positions.push_back(glm::vec3((positions[3][0] + positions[1][0]) / 2.0,
                                  (positions[3][1] + positions[1][1]) / 2.0,
                                  (positions[3][2] + positions[1][2]) / 2.0));
